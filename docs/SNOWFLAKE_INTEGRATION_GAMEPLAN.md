# Snowflake Integration Gameplan — InsightHub

**Prepared for:** Meeting with Head of Data Analytics  
**Date:** April 22, 2026  
**Author:** Jeff Coy  
...
6. Meeting discussion topics and open questions
...
These controls are enforced at both the **API layer** (server-side, before data is returned) and the **AI prompt layer** (restricted categories are injected into the system prompt so Claude cannot generate queries against them).

### 1.4 Snowflake-Specific Security Layers (Already Built)

#### SQL Injection Prevention (`query-executor.ts`)
...
#### Row-Level Security (`row-level-security.ts`)
...
#### Column-Level Data Masking (`data-security.ts`)
...
- **OCSP certificate validation** — enabled in production (`insecureConnect: false`)
- **Graceful shutdown** — connections cleaned up on process exit and SIGINT

#### Schema Introspection (`schema.ts`)
...
- **3-layer verification** of AI-generated dashboard configurations:
  ...
  - Layer 2: AI semantic verification via Claude Haiku (~$0.002, 2-4s)
  - Layer 2.5: Escalation to Claude Sonnet if confidence < 0.70
...
- **EC2 deployment** behind Nginx reverse proxy (not a public cloud platform like Vercel/Netlify)
...
- **Gradual rollout** — you can enable Snowflake for specific users/roles first
- **Safe demos** — switch between live and sample data by toggling env vars
...
| 2 | **Key-pair authentication** | 1 day | Replace password-based auth with RSA key-pair authentication. The codebase currently uses `SNOWFLAKE_PASSWORD`; this should be upgraded to `SNOWFLAKE_PRIVATE_KEY` / `SNOWFLAKE_PRIVATE_KEY_PASSPHRASE`. |
...
| 4 | **Table mapping configuration** | 2-3 days | Map InsightHub data source names to actual Snowflake table paths. Current mapping is a stub in `snowflake-data-provider.ts:151-158`. Needs real table names from the analytics team. |
| 5 | **SQL injection fixes** | 2-3 days | Resolve 3 identified injection vectors: (a) unmapped source names in `buildSnowflakeQuery()`, (b) column name interpolation in `SnowflakeQueryBuilder`, (c) formula expressions in visual-to-SQL. All documented in CISO report as risks #14-16. |
| 6 | **Unified PII detection layer** | 2-3 days | Consolidate the two separate PII stripping layers (name-match for sample data + regex-based for Snowflake) into a single centralized pipeline that handles column aliasing. |
| 7 | **Secrets management upgrade** | 1-2 days | Move Snowflake credentials from `.env.local` file to AWS Secrets Manager or SSM Parameter Store. Current approach copies the env file via SCP during deploy. |
| 8 | **Dev mode enforcement** | < 1 day | Re-add fatal enforcement when `NEXT_PUBLIC_DEV_MODE=true` in production, preventing authentication bypass. Currently downgraded to warning-only. |
...
| # | Item | Effort | Description |
|---|------|--------|-------------|
| 14 | **Snowflake views layer** | 3-5 days | Create pre-approved Snowflake views that serve as the "contract" between raw data and InsightHub. The app queries views, not raw tables. This adds a transformation/abstraction layer controlled by the data team. |
...
## Part 4: Recommended Snowflake Integration Approach

### 4.1 The "Views Layer" Strategy (Recommended)

Rather than pointing InsightHub directly at raw Snowflake tables, we recommend creating a **curated views layer**:
...
**Why this approach wins stakeholder confidence:**
...
- Fix SQL injection vectors (#14-16 from CISO report)
- Move credentials to AWS Secrets Manager
- **Milestone:** Successful `SELECT 1` health check from EC2 to Snowflake
...
## Part 5: How We Assure Stakeholders the Data Is Safe

### 5.1 Defense in Depth — 7 Security Layers
...
### 5.2 What InsightHub CANNOT Do to Snowflake
...
### 5.3 What the Data Team Controls (Not InsightHub)
...
**Key message for the meeting:** The Data Team retains full control over what data enters InsightHub. They control the views, the service account permissions, and the network policies. InsightHub is a **consumer** of pre-approved data, not an admin of the data warehouse.

---

## Part 6: Meeting Agenda — Topics & Questions to Discuss

### 6.1 Topics You Should Cover

1. **Demo the application** — Show InsightHub with sample data. Let him see the dashboard builder, AI chat, and the RBAC admin panel.

2. **Walk through the security architecture** — Use the 7-layer diagram from Section 5.1. Emphasize defense-in-depth.

3. **Explain the Views Layer strategy** — This is the key selling point. His team controls what data is exposed.

4. **Show the CISO Report** — Reference the existing `docs/CISO_REPORT.md` — a thorough security audit has already been conducted.

5. **Discuss the phased rollout** — Start small (3 views, admin-only), expand as confidence grows.

6. **Address the "AI reading our data" concern** — Claude (the AI) never sees raw Snowflake data. It generates widget configurations based on data source names and column metadata. The actual query execution happens server-side with all security layers applied. Claude sees aggregated results only when the Data Integrity Verification pipeline checks output quality.

### 6.2 Questions to Ask Him

#### Data Access & Governance
- **Which Snowflake database(s) and schema(s) contain the analytics data we'd want to surface?**
- **Does your team already have a curated views/marts layer, or would we need to build one?**
...
- **Do you have existing data classification standards?** (What's considered PII, confidential, restricted?) — We can map our sensitivity tags to their taxonomy.
...
- **Who approves new service accounts and grants on Snowflake?** — We need to understand the provisioning process.
- **Are there existing Snowflake network policies we need to work within?**
- **Does the security team need to review our CISO report before we proceed?**
- **Is there a formal change management process for connecting new applications to Snowflake?**

#### Technical
- **What Snowflake edition are you on?** (Standard, Enterprise, Business Critical) — Enterprise+ has native masking policies we could leverage.
- **Do you use Snowflake's native Dynamic Data Masking or Row Access Policies?** — If yes, we get an additional security layer for free.
- **What are typical query volumes and data sizes we'd be working with?** — Helps size the warehouse and cache layer.
- **Is there an existing Redis or caching infrastructure we can use, or should we deploy our own?**
- **Do you have monitoring/alerting on Snowflake query costs?** — We need resource monitors.

#### Organizational
- **Who from your team would be our point of contact for the views layer?** — Someone who can create/modify the views as we add new data sources.
- **What's the approval process for adding new views/data sources?** — We need a defined workflow.
- **Are there other internal tools already connected to Snowflake?** — Understanding precedent helps.
- **What would make you feel confident enough to approve this connection?** — This is the most important question. Let him define success criteria.

### 6.3 Potential Objections & Responses

| Objection | Response |
|-----------|---------|
| "AI is reading our data" | Claude generates widget configs from metadata. It never receives raw row-level data. The actual Snowflake queries are executed server-side by parameterized SQL. |
| "What if there's a SQL injection?" | We have 4 layers of protection: parameterized queries, dangerous pattern blocking, read-only enforcement, and the service account itself is read-only. Even if all app-level protections failed, the Snowflake account can't modify data. |
| "What if someone bypasses auth?" | Google OAuth + domain restriction + JWT. Even if bypassed, RBAC still controls data access, and Snowflake views limit what's queryable. |
| "We can't expose raw tables" | We don't want to. The views layer means your team controls exactly what's exposed. |
| "What about costs?" | XS warehouse with auto-suspend (60s). Redis cache reduces repeat queries. Resource monitors alert at thresholds. Typical cost: $2-5/day for moderate usage. |
| "What if the app has a bug?" | Snowflake is the last line of defense: read-only service account, network policy, views-only access. The worst case is a user sees data they're authorized for at the Snowflake level. |
| "We need an audit trail" | Every query is logged with user identity, SQL text, execution time, and row count. We're happy to ship these logs wherever your team requires. |
...
|------|-----------|--------|------------|--------|
| SQL injection in Snowflake queries | Low | Critical | Fix 3 identified vectors before launch | **Open** |
...
## Appendix A: Files Relevant to Snowflake Integration

| File | Purpose |
|------|---------|
| `src/lib/snowflake/config.ts` | Snowflake connection configuration |
| `src/lib/snowflake/connection.ts` | Connection pool management |
| `src/lib/snowflake/query-executor.ts` | Secure query execution with validation |
| `src/lib/snowflake/data-security.ts` | Column-level masking and PII detection |
| `src/lib/snowflake/row-level-security.ts` | Row-level security policy engine |
| `src/lib/snowflake/schema.ts` | Schema introspection and discovery |
| `src/lib/data/snowflake-data-provider.ts` | Unified data provider with fallback |
| `src/lib/auth/permissions.ts` | RBAC and metric-level access control |
| `src/lib/redis/client.ts` | Query result caching |
| `src/lib/audit.ts` | Audit logging |
| `docs/CISO_REPORT.md` | Full security audit report |
| `docs/DATA_INTEGRITY_VERIFICATION_SPEC.md` | AI output verification pipeline |
...
SNOWFLAKE_PASSWORD=""                    # Replace with key-pair auth
SNOWFLAKE_WAREHOUSE="INSIGHTHUB_WH"
SNOWFLAKE_DATABASE="ANALYTICS"
SNOWFLAKE_SCHEMA="INSIGHTHUB_ANALYTICS"
SNOWFLAKE_ROLE="INSIGHTHUB_READER"

# Redis Cache (recommended for production)
REDIS_URL="rediss://your-redis-host:6379"  # Note: rediss:// for TLS
REDIS_PASSWORD="your-redis-password"
```

---

*This document should be treated as a living artifact. Update it after the meeting with the Head of Data Analytics based on decisions made and action items assigned.*
