...
- [ ] **[Review Fix] Public health endpoint also uses PrismaClient-per-request (pre-existing)**
- [ ] **[Review Fix] Publish widget route accepts publisherId from request body while authenticated**
- [ ] **[Review Fix] Auth error detection relies on fragile string matching**
- [ ] **[Review Fix] Dead-code null check after getCurrentUser() in all hardened routes**
- [ ] **[Review Fix] Admin health endpoint creates new PrismaClient per request**
- [ ] **[Review Fix] Glossary list endpoint still has no authentication**
...
- [ ] **🛡️ USZoom Policy Compliance — Gap Remediation**
  - [ ] [G-38] [T3 · LOW · S] Move ADMIN_EMAILS hardcode into DB-backed AdminRoleAssignment table
  - [x] [G-37] [T3 · HIGH · M] Statement of Applicability for ISO 27001:2022 (covered by G-34)
  - [ ] [G-36] [T3 · MED · M] Migrate production secrets to AWS Secrets Manager + 90-day rotation policy
  - [ ] [G-35] [T3 · LOW · S] Track annual security awareness training record in HR
  - [x] [G-34] [T3 · HIGH · M] Write docs/RISK_REGISTER.md + docs/STATEMENT_OF_APPLICABILITY.md
  - [ ] [G-33] [T3 · MED · S] Add /privacy page + CCPA/SHIELD/CalOPPA compliance UI
  - [ ] [G-32] [T3 · MED · L] Commission third-party penetration test (~$5-15k)
  - [ ] [G-31] [T3 · MED · M] Write DR plan with RTO 4h / RPO 24h + annual DR test
  - [ ] [G-30] [T3 · LOW · L] DLP program (accept risk for current scope)
  - [ ] [G-29] [T3 · LOW · S] Subscribe to threat-intel feeds + document intake process
  - [ ] [G-28] [T3 · LOW-MED · M] File Integrity Monitoring on EC2 (AIDE or Wazuh agent)
  - [ ] [G-27] [T2 · MED · M] Automated scripts/test-restore.sh + annual restore-test evidence
  - [ ] [G-26] [T2 · MED-HIGH · S] Create docs/VENDOR_REGISTER.md with DPAs + SOC reports + annual review
  - [ ] [G-25] [T2 · LOW · S] Write docs/RELEASE_CHECKLIST.md + link from deploy.sh
  - [ ] [G-24] [T2 · MED · S] Add SAST (Semgrep or CodeQL) to CI
  - [ ] [G-23] [T2 · MED · M] Enable AWS Inspector + Config + quarterly nmap vuln scans
  - [ ] [G-22] [T2 · MED-HIGH · L] Stand up staging environment + rename dev.db→prod.db + env badge in UI
  - [ ] [G-21] [T2 · MED · M] CloudWatch alarms for CPU/disk/memory/5xx + alerting wiring
  - [ ] [G-20] [T2 · MED · S] Add structured ipAddress/userAgent to AuditLog + metadata sanitizer
  - [ ] [G-19] [T2 · MED-HIGH · S] Enable branch protection on main + PR review (or document solo-dev accepted risk)
  - [x] [G-18] [T2 · HIGH · M] Write Incident Response Runbook + annual tabletop using policy 6458 scenarios
  - [ ] [G-17] [T2 · LOW · S] SSH idle timeout via sshd_config ClientAliveInterval
  - [ ] [G-16] [T2 · MED · L] Capture AWS infra in Terraform/CDK (Security Group, EBS, IAM, Route53)
  - [ ] [G-15] [T2 · MED · S] Enable Dependabot + unattended-upgrades + document patching SLAs
  - [ ] [G-14] [T2 · MED · M] Apply CIS Level-1 hardening baseline + capture report
  - [ ] [G-13] [T2 · HIGH · M] Cross-region isolated backups + enforced backup encryption + Secrets Manager for BACKUP_ENCRYPTION_KEY
  - [ ] [G-12] [T2 · MED · S] Verify EBS/S3 encryption in deploy.sh pre-flight
...
- [x] **🛡️ Data Integrity Verification Pipeline — AI-Generated Dashboard Accuracy Checks**
  - [x] 5️⃣ Chat Route + ChatPanel Integration + UX Badges
  - [x] 4️⃣ Layer 2.5 — Escalation Review + Orchestrator
  - [x] 3️⃣ Layer 2 — AI Verification (Haiku semantic check)
  - [x] 2️⃣ Layer 1 — Deterministic Checks (D-01 through D-13)
  - [x] 1️⃣ Source-Field Registry + Type Definitions
...
- [x] **📁 Folder Power Features — Aliasing, Sort Modes, Drag-Hover Expand, Picker UI**
...
- [x] **🔧 Deferred: Visual Query Builder — needs full QA pass before shipping**
