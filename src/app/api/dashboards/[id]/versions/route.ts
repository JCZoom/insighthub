...
export async function GET(request: NextRequest, context: RouteContext) {
  return withRateLimit(request, dashboardRateLimiter, 'dashboards:versions:list', async () => {
    try {
...
export async function POST(request: NextRequest, context: RouteContext) {
  return withRateLimit(request, dashboardRateLimiter, 'dashboards:versions:save', async () => {
    try {
      const { id } = await context.params;
      const user = await getCurrentUser();
      ...
      // Log version save for audit
      await logVersionAction(
        user.id,
        AuditAction.VERSION_SAVE,
        version.id,
        {
          dashboardId: id,
