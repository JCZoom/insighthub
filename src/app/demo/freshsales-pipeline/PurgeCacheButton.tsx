...
/**
 * Live retention-purge button for the Freshsales demo page.
 *
 * Calls `POST /api/admin/retention` with `target=freshworks_cache, dryRun=false`.
 * Surfaces the result count in-place and prompts a page reload so the user
 * can see the cache rebuild from a fresh API hit.
 *
 * Admin-only — the parent page checks `isAdmin(user)` before rendering.
 */
  ...
  async function purge() {
    ...
    try {
      ...
      const data = (await res.json()) as {
        freshworksCache?: { deleted: number };
      };
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        >
          {status === 'purging' ? 'Purging…' : 'Purge Freshsales cache now'}
        </button>
        {status === 'done' && result && (
          <span className="text-sm text-emerald-700 dark:text-emerald-300">
            ✅ Deleted <strong>{result.deleted}</strong> cache key{result.deleted === 1 ? '' : 's'}.
            Reload the page to re-fetch from Freshsales.
          </span>
        )}
        {status === 'error' && error && (
